...
async fn proposer_crashes_after_proposing() {
    ...
    struct State {
        ...
        actual_crash_height: Option<u64>,
    }
    ...
    test.add_node()
        ...
        // Check that we replay messages from the WAL at the actual crash height
        .expect_wal_replay_at_crash_height(|state| state.actual_crash_height)
...
async fn non_proposer_crashes_after_voting() {
    ...
    test.add_node()
        ...
        // Crash immediately to minimize height progression
        .crash()
        // Restart after 5 seconds
        .restart_after(Duration::from_secs(5))
