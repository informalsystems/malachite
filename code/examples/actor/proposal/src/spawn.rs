...
pub async fn spawn_node_actor(
    ...
    let network: ractor::ActorRef<malachitebft_engine::network::Msg<MockContext>> =
        spawn_network_actor(&cfg, &private_key, &registry, &span).await;
...
async fn spawn_consensus_actor(
    ...
    let consensus_params = ConsensusParams {
        ...
        value_payload: match cfg.consensus.value_payload {
            ...
            config::ValuePayload::PartsOnly | config::ValuePayload::ProposalAndParts => {
                panic!(
                    "PartsOnly and ProposalAndParts modes are not supported for actor-app-proposal"
                )
            }
        },
    };
